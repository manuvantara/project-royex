...
def get_royalty_token(session: Session, is_live: bool) -> List[RoyaltyToken]:
    ...
    royalty_symbols: List[str] = tuple(
        set(
            [
                initial_royalty_offering.royalty_token_symbol
                for initial_royalty_offering in initial_royalty_offerings
            ]
        )
    )
    royalty_prices: Dict[str, List] = dict(
        zip(royalty_symbols, [[] for _ in range(len(royalty_symbols))])
    )
    ...
    for hour in hour_timestamps:
        for royalty_symbol in royalty_symbols:
            ...
            statement = (
                ...
                .where(
                    RoyaltyTokenSoldEvent.contract_address
                    == royalty_exchange.contract_address,
            ...
            statement = (
                ...
                .where(
                    RoyaltyTokenBoughtEvent.contract_address
                    == royalty_exchange.contract_address,
            ...
            statement = select(RoyaltyPoolDepositedEvent).where(
                ...
                == royalty_payment_pool.contract_address,
                RoyaltyPoolDepositedEvent.block_timestamp <= int(hour),
            )
            royalty_pool_deposited_events = session.exec(statement).all()

            transaction = (
                royalty_token_sold_event
                if royalty_token_sold_event.block_timestamp
                > royalty_token_bought_event.block_timestamp
                else royalty_token_bought_event
            )
            deposit = sum(
                [deposit.deposit for deposit in royalty_pool_deposited_events]
            )

            royalty_prices[royalty_symbol].append(
                transaction.royalty_token_amount / transaction.stablecoin_amount
            )
...
@router.get("/live")
def fetch_live(*, session: Session = Depends(get_session)) -> List[RoyaltyToken]:
    return get_royalty_token(session, is_live=True)


@router.get("/upcoming")
def fetch_upcoming(*, session: Session = Depends(get_session)) -> List[RoyaltyToken]:
